...
}document.getElementById("imageOut").src=canvas.toDataURL("image/png")},imageObj.src="https://crossorigin.me/"+$scope.url},$scope.getLines=function(ctx,text,maxWidth){for(var words=text.split(" "),lines=[],currentLine=words[0],i=1;i<words.length;i++){var word=words[i],width=ctx.measureText(currentLine+" "+word).width;maxWidth>width?currentLine+=" "+word:(lines.push(currentLine),currentLine=word)}return""!=currentLine&&lines.push(currentLine),lines},$scope.instagram.isSelected=function(id){return $scope.instagram.selected==id},$scope.instagram.search()}]);